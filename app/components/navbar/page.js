...
export default function Navbar() {
  ...
  return (
    <div className="bg-[#191924] h-20 flex items-center justify-center text-base sticky top-0 z-50">
      <div className="flex justify-between items-center h-16 w-full px-6 max-w-[1200px]">
        <div className="flex justify-start items-center w-4/5 px-1">
          <Link href="/" className="font-bold text-lg text-[#F2F3F4]">
      ...
      {
        isOpen &&
        <div className="flex flex-col justify-center gap-4 absolute top-20 right-0 w-full px-10 py-6 bg-[#191924] transition-all duration-600 ease-in-out rounded-b-2xl shadow-md opacity-100 z-50">
          <Link className="text-[#F2F3F4] font-medium cursor-pointer transition-all duration-200 ease-in-out hover:text-[#854CE6]" href="/about" onClick={() => setIsOpen(false)}>About</Link>
          <Link className="text-[#F2F3F4] font-medium cursor-pointer transition-all duration-200 ease-in-out hover:text-[#854CE6]" href='/skills' onClick={() => setIsOpen(false)}>Skills</Link>
          <Link className="text-[#F2F3F4] font-medium cursor-pointer transition-all duration-200 ease-in-out hover:text-[#854CE6]" href='/projects' onClick={() => setIsOpen(false)}>Projects</Link>
          <Link className="text-[#F2F3F4] font-medium cursor-pointer transition-all duration-200 ease-in-out hover:text-[#854CE6]" href='/contact' onClick={() => setIsOpen(false)}>Contact</Link>
        </div>
      }
    </div>
  );
